...
// Configuración actual: 25 preguntas, 75 minutos
...
class SimuladorExamen {
    ...
    mostrarPregunta() {
        ...
        if (this.modo === 'examen') {
            ...
            const totalEnBloque = Math.min(10, this.preguntas.length - ((bloque - 1) * 10));
            ...
            document.getElementById('preguntaNumero').textContent =
                `Bloque ${bloque}/${totalBloques} — Pregunta ${enBloque}/${totalEnBloque} (global ${this.preguntaActual + 1}/${this.preguntas.length})${indicadorNoResponder}`;
        } else {
            document.getElementById('preguntaNumero').textContent =
    ...
    mostrarResultados() {
        ...
        const aciertosNetos = correctas - (incorrectas * PENALIZACION_ERROR);
        const puntuacionFinal = totalPreguntas > 0
            ? Math.max(0, Math.min(10, (aciertosNetos / totalPreguntas) * 10))
            : 0;
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    const totalPreguntasActual = document.getElementById('totalPreguntasActual');
    if (totalPreguntasActual) totalPreguntasActual.textContent = TOTAL_PREGUNTAS_PRACTICA;
