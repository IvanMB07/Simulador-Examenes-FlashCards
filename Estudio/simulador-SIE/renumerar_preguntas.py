...
# python Estudio/simulador-SIE/renumerar_preguntas.py --input Estudio/simulador-SIE/preguntas.json --output Estudio/simulador-SIE/preguntas-renumerado.json
...
def build_group_map(items):
    ...
    for item in items:
        ...
        if len(parts) >= 2 and parts[0] == "SIE":
            parts[1] = f"EDep{format_index(counter)}"
            group_map[group_key] = "-".join(parts)
        else:
            group_map[group_key] = f"SIE-EDep{format_index(counter)}-{group_key}"
    return group_map


def extract_tema(question: str):
    match = re.match(r"^SIE-(T\d+)", question)
...
def renumber(items, group_map, theme_order):
    ...
    for _, item, tema in indexed:
        question = item.get("cuestion", "")
        if tema and "-Pregunta:" in question and question.startswith("SIE-"):
            counters[tema] += 1
            number = format_index(counters[tema])
            _, rest = question.split("-Pregunta:", 1)
            item["cuestion"] = f"SIE-{tema}-{number}-Pregunta:{rest}"
